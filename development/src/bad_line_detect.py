...
class BadLineDetect(Entity):
	...
	def _run(self):
		if self.i_clk.posedge():
			...
			if (self.i_strb == 10):
				if (self.i_cycl == specs.CYCL_REF - 1):
					self.o_bdln <<= 0
				else:
					self.o_bdln <<= 0
					if ((self.i_ypos >= 48) and (self.i_ypos < 248) and
					    (self.i_cycl >= specs.CYCL_REF                ) and
					    (self.i_cycl < specs.CYCL_REF + 40            )):
						self.o_bdln <<= not self.i_aec
