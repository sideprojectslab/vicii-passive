	...
	def _run(self):
		...
		for _ in range(2):
			for l in self.lines:
				...
				self.dut.i_ph0 <<= (line_val >> 21) & 1
				self.dut.i_db  <<= (line_val >> 9) & (2**12 - 1)
				self.dut.i_a   <<= (line_val >> 3) & (2**6 - 1)
				self.dut.i_rw  <<= (line_val >> 2) & 1
				self.dut.i_cs  <<= (line_val >> 1) & 1
				self.dut.i_aec <<= (line_val >> 0) & 1
